...
if [ ! -f "app.py" ]; then
    ...
    # 下载 app.py（优先 curl/wget）
    DOWNLOAD_URL="https://raw.githubusercontent.com/YGGYDW/lunes.host-py/refs/heads/main/app.py"
    if command -v curl &> /dev/null; then
        curl -fsSL "$DOWNLOAD_URL" -o app.py || true
    elif command -v wget &> /dev/null; then
        wget -q "$DOWNLOAD_URL" -O app.py || true
    else
        if is_linux && command -v apt-get &> /dev/null; then
            warn "未检测到 curl 或 wget，尝试通过 apt-get 安装 wget..."
            sudo DEBIAN_FRONTEND=noninteractive apt-get install -y wget || true
            wget -q "$DOWNLOAD_URL" -O app.py || true
        else
            err "系统未安装 curl/wget，无法自动下载 app.py，请手动下载到当前目录。"
        fi
    fi

    if [ ! -f "app.py" ]; then
        err "下载失败，请检查网络连接或手动将 app.py 放到当前目录。"
        exit 1
    fi
